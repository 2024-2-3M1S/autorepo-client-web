...
import { RxHamburgerMenu, RxCross1 } from 'react-icons/rx'; // Import a cross icon for closing
/**
...
export default function Navigation() {
    ...
    const NavgationList: NavItem[][] = [
        [
            ...
            { label: 'Issue/PR 템플릿 생성', link: '/iptemplate' },
        ],
        [{ label: 'ReadMe 생성', link: '/readme' }],
        [{ label: '3분 1초란?', link: '/introduction' }],
        isUserLogin
            ? [{ label: '프로필', link: '/profile' }]
            : [{ label: '로그인', link: '/login' }],
    ];

    return (
        <>
            <div className=" fixed z-[99999] w-lvw flex flex-col justify-center items-center bg-netural-100">
                <div className="w-lvw flex justify-between items-center tablet:px-16 py-8 px-8">
                    ...
                    {isSideBarOpen ? (
                        <RxCross1
                            className="w-6 h-6 desktop:hidden"
            ...
            {isModalOpen ? (
                <div
                    className={`fixed left-0 top-0 z-0 w-lvw h-lvh mix-blend-multiply  ${
                        isSideBarOpen
                            ? ' desktop:bg-transparent bg-mono_200'
                            : ''
                    }`}
